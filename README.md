...
1. **Scene 1: Multi-modal intent & ToF environmental scan** (0:00-0:08). "Ducky, navigate the maze, retrieve the marker, and drop it in the tray!" parsed via stateless one-shot extractor. 8x8 matrix ToF active depth scan with 64 color-coded live cells on HUD.
2. **Scene 2: Ground crouch retrieval with zero floor penetration** (0:08-0:16). Ground approach and authentic bipedal squat derived from Pollen Robotics official `alpha_ground_pick.onnx` policy kinematics with foot soles strictly flat on the floor ($z_{\text{foot}} \ge 0.000\,\text{m}$, zero warping), and $C^2$ quintic smoothstep transitions.
3. **Scene 3: Articulated beak clamp & counterbalanced payload lift** (0:16-0:24). Authentic Pollen lower jaw articulates open to $28\,\text{mm}$ aperture ($0.35\,\text{rad}$), clamps firmly on $14\,\text{mm}$ dry-erase marker with active collision geometry, and lifts to full upright stance with dynamic center-of-mass counterbalancing and BAM M6 battery voltage sag telemetry.
4. **Scene 4: Desktop maze chicane navigation with real-time ToF avoidance** (0:24-0:32). Navigates a staggered 2-wall desktop wooden maze chicane with real-time 8x8 ToF obstacle proximity steering and living anticipatory head yaw saccades into upcoming turns.
5. **Scene 5: Shallow transparent container arrival & marker drop-off** (0:32-0:40). Terminal arrival at shallow acrylic container ($12\,\text{mm}$ clear rim), forward bowing posture over target, beak opens to $28\,\text{mm}$, and marker settles naturally under gravity ($g = -9.81\,\text{m/s}^2$) with a plastic drop acoustic click.
6. **Scene 6: Celebratory tilt, cheerful quack & seated rest** (0:40-0:48). Expressive $+14^\circ$ inquisitive head roll celebration tilt, nod, cheerful vocal quack, and smooth descent into seated rest posture. Unanimously certified zero falls across all 1,440 frames, 100% factory XL330 Dynamixel limits observed, all 5 evaluation benchmarks passed.
